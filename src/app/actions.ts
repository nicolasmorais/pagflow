...
export async function sendPixEmail(orderId: string, qrCode: string, qrCodeBase64: string) {
    try {
        ...
        try {
            ...
            console.error('FAILED TO RECORD PIX EMAIL LOG:', logErr);
        }

        if (error) {
            console.error('RESEND PIX EMAIL ERROR:', JSON.stringify(error, null, 2));
            return { success: false, error: error.message || 'Falha ao enviar e-mail PIX' };
        }

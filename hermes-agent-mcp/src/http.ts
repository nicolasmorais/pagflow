...
app.get('/api/orders', async (req, res) => {
  try {
    const { from, to, status, paymentStatus, paymentMethod, productId, email, limit, offset } = req.query as Record<string, string | undefined>
    const where: any = dateFilter(from, to)
...
app.get('/api/sales-by-period', async (req, res) => {
  try {
    const { from, to, groupBy = 'day', productId } = req.query as Record<string, string | undefined>
    const where: any = dateFilter(from, to)
    where.paymentStatus = 'pago'
    if (productId) where.productId = productId
...
app.get('/api/utm-performance', async (req, res) => {
  try {
    const { from, to, groupBy = 'source' } = req.query as Record<string, string | undefined>
    const where: any = dateFilter(from, to)
    where.paymentStatus = 'pago'
...
app.get('/api/financial', async (req, res) => {
  try {
    const { from, to, type, category } = req.query as Record<string, string | undefined>
    const where: any = {}
    if (from || to) { where.date = {}; if (from) where.date.gte = new Date(from); if (to) where.date.lte = new Date(to + 'T23:59:59') }
...
app.get('/api/dashboard-kpis', async (req, res) => {
  try {
    ...
    for (const o of paid) { byMethod[o.paymentMethod || 'outro'] = (byMethod[o.paymentMethod || 'outro'] || 0) + 1 }
...
app.get('/api/customers', async (req, res) => {
  try {
    ...
    for (const o of orders) {
      ...
      if (o.createdAt > customers[key].ultima_compra) customers[key].ultima_compra = o.createdAt
      if (o.createdAt < customers[key].primeira_compra) customers[key].primeira_compra = o.createdAt
    }
...
app.get('/api/email-logs', async (req, res) => {
  try {
    const { from, to, type, status, limit } = req.query as Record<string, string | undefined>
    const where: any = {}
    if (from || to) { where.sentAt = {}; if (from) where.sentAt.gte = new Date(from); if (to) where.sentAt.lte = new Date(to + 'T23:59:59') }
    if (type) where.type = type
    if (status) where.status = status

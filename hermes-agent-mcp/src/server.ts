...
export function createServer() {
  ...
  function dateFilterSales(from?: string, to?: string) {
    const where: any = {}
    if (from || to) {
      where.created_at = {}
  ...
  server.tool(
    'get_sales_summary',
    'Resumo de vendas com métricas agregadas (total, ticket médio, por status, por método de pagamento). Use para entender o desempenho geral de vendas.',
    {
  ...
  server.tool(
    'get_orders',
    'Lista pedidos com filtros. Retorna dados do cliente, produto, pagamento, rastreamento e UTM.',
    {
      from: z.string().optional().describe('Data inicial (YYYY-MM-DD)'),
      to: z.string().optional().describe('Data final (YYYY-MM-DD)'),
  ...
  server.tool(
    'get_order_detail',
    'Detalhes completos de um pedido específico, incluindo dados do cliente, endereço, produto, bumps, pagamento e logs de e-mail.',
    {
    ...
    async ({ orderId }) => {
      ...
      if (!order) {
        return { content: [{ type: 'text' as const, text: 'Pedido não encontrado.' }] }
      }
  ...
  server.tool(
    'get_products',
    'Lista todos os produtos com preços, custos, comissão e contagem de vendas.',
    {},
    async () => {
  ...
  server.tool(
    'get_top_products',
    'Ranking dos produtos mais vendidos com receita, ticket médio e taxa de conversão.',
    {
      from: z.string().optional().describe('Data inicial (YYYY-MM-DD)'),
      to: z.string().optional().describe('Data final (YYYY-MM-DD)'),
    ...
    async ({ from, to, limit }) => {
      ...
      for (const o of orders) {
        ...
        if (!grouped[pid]) grouped[pid] = { count: 0, revenue: 0 }
        grouped[pid].count++
  ...
  server.tool(
    'get_sales_by_period',
    'Vendas agrupadas por dia, semana ou mês. Útil para gráficos de tendência.',
    {
  ...
  server.tool(
    'get_utm_performance',
    'Performance de campanhas UTM (source, medium, campaign). Mostra vendas, receita e ticket médio por campanha.',
    {
      from: z.string().optional().describe('Data inicial (YYYY-MM-DD)'),
      to: z.string().optional().describe('Data final (YYYY-MM-DD)'),
    ...
    async ({ from, to, groupBy }) => {
      ...
      for (const o of orders) {
        let key: string
        if (groupBy === 'source') key = o.utmSource || 'direto'
        else if (groupBy === 'medium') key = o.utmMedium || 'direto'
        else key = o.utmCampaign || 'direto'
  ...
  server.tool(
    ...
    'Registros financeiros (receitas e despesas). Filtrar por tipo, categoria e período.',
    {
      from: z.string().optional().describe('Data inicial (YYYY-MM-DD)'),
      to: z.string().optional().describe('Data final (YYYY-MM-DD)'),
  ...
  server.tool(
    'get_dashboard_kpis',
    'KPIs do dashboard: receita total, ticket médio, taxa de conversão, vendas por status, comparação com período anterior.',
    {
      from: z.string().optional().describe('Data inicial (YYYY-MM-DD)'),
      to: z.string().optional().describe('Data final (YYYY-MM-DD)'),
    },
  ...
  server.tool(
    'get_customers',
    'Lista de clientes únicos com total de pedidos e gasto. Útil para identificar clientes recorrentes.',
    {
      from: z.string().optional().describe('Data inicial (YYYY-MM-DD)'),
      to: z.string().optional().describe('Data final (YYYY-MM-DD)'),
  ...
  server.tool(
    ...
    'Funil de conversão: quantos pedidos em cada estágio (pendente → pago → enviado). Inclui taxa de conversão.',
    {
      from: z.string().optional().describe('Data inicial (YYYY-MM-DD)'),
      to: z.string().optional().describe('Data final (YYYY-MM-DD)'),
      productId: z.string().optional().describe('Filtrar por produto'),
  ...
  server.tool(
    'get_sales_table',
    'Dados da tabela de vendas (tabela sales do banco). Diferente de orders — esta é a tabela de vendas legada com dados de produto e UTM.',
    {
      from: z.string().optional().describe('Data inicial (YYYY-MM-DD)'),
      to: z.string().optional().describe('Data final (YYYY-MM-DD)'),
    ...
    async ({ from, to, status, limit }) => {
      const where: any = dateFilterSales(from, to)
      if (status) where.status = status
  ...
  server.tool(
    'search_orders',
    'Busca pedidos por nome, email, CPF, ID do pagamento MP ou código de rastreio.',
    {
  ...
  server.tool(
    'get_email_logs',
    'Logs de e-mails enviados. Mostra quais e-mails foram enviados, status e erros.',
    {
      from: z.string().optional().describe('Data inicial (YYYY-MM-DD)'),
      to: z.string().optional().describe('Data final (YYYY-MM-DD)'),
